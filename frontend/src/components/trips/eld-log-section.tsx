...
function getDayStartUTC(logDateStr: string): Date {
  return new Date(logDateStr + "T00:00:00Z");
}

function getHoursFromDayStart(dateStr: string, dayStart: Date): number {
...
function formatTimeRange(arrivalStr: string, departureStr: string): string {
...
function formatStopDuration(hours: number): string {
...
function getStopDutyLabel(stopType: string, durationStr: string): string {
...
export const EldLogSection: React.FC<EldLogSectionProps> = ({ trip }) => {
  ...
  const activeDailyLog = useMemo(() => {
    return trip.eld_daily_logs.find((log) => log.day_number === activeDay) || trip.eld_daily_logs[0];
  ...
  const dayStart = useMemo(
    () => getDayStartUTC(activeDailyLog.log_date),
    [activeDailyLog.log_date]
  );

  // Active day duty log segments
  const dayEntries = useMemo(() => {
    return (trip.duty_log_entries || [])
      .filter((entry) => entry.trip_day === activeDay)
  ...
  const polylinePoints = useMemo(() => {
    ...
    dayEntries.forEach((entry, idx) => {
      ...
      if (idx === 0) {
        ...
        const prevY = statusToY(dayEntries[idx - 1].duty_status);
        if (prevY !== y) {
          points.push(`${xStart.toFixed(1)},${y}`);
        }
      }
              ...
              return (
                <div
                  ...
                  className={`flex gap-4 py-3 items-center ${
                    idx < trip.stops.length - 1 ? "border-b border-[#1e2438]" : ""
                  }`}
                >
                  {/* Category icon */}
                  <div
